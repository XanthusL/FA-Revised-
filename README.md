GreenLens — Final Project Submission
C240 AI Essentials and Innovation

LIVE LINKS
----------
Website (source code + GitHub repo):
https://xanthusl.github.io/FA-Revised-/Website/video.html

Live website (GitHub Pages):
https://xanthusl.github.io/FA-Revised-/

Promotional video:
https://youtu.be/pZVDysHLbOU

CONTENTS OF THIS ZIP
---------------------
- Website source code (HTML/CSS/JS)
- n8n workflow export (JSON)
- Report
- Slide deck
- Promotional materials (poster, branding assets; video hosted externally, see link above)
- Individual contribution statements

TEAM
----
Liew Qi Xuan Xanthus — Solution Lead,                     

Bryan Ee Yong Le — Data Lead,                    

Ge Youyou — Media Lead,                  

Klauzann Tan Jing Ting — Comms Lead,               

Lim Yi Ming — QA/Testing Lead
